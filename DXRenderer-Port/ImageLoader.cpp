...
winrt::DXRenderer::ImageInfo ImageLoader::LoadImageFromDirectXTex(wstring filename, wstring extension)
{
    ...
    com_ptr<IWICBitmapSource> decodedSource;

    auto dxtScratch = new ScratchImage();

    if (extension == L".EXR" || extension == L".exr")
    {
        CHK(LoadFromEXRFile(filename.c_str(), nullptr, *dxtScratch));
    }
    else if (extension == L".HDR" || extension == L".hdr")
    {
        CHK(LoadFromHDRFile(filename.c_str(), nullptr, *dxtScratch));
    }
    else
    {
        CHK(LoadFromDDSFile(filename.c_str(), 0, nullptr, *dxtScratch));
    }
    ...
    com_ptr<IWICBitmap> dxtWicBitmap;
...
void ImageLoader::LoadImageCommon(_In_ IWICBitmapSource* source)
{
    ...
    CHK(source->GetPixelFormat(&pixelFormat));

    com_ptr<IWICComponentInfo> componentInfo;
    CHK(wicFactory->CreateComponentInfo(pixelFormat, componentInfo.put()));
    ...
    CHK(pixelFormatInfo->GetNumericRepresentation(&formatNumber));
    ...
    CHK(pixelFormatInfo->GetChannelMask(
    ...
    CHK(
        wicFactory->CreateFormatConverter(m_formatConvert.put())
    );
    ...
    UINT width;
    UINT height;
    CHK(m_formatConvert->GetSize(&width, &height));
...
void ImageLoader::CreateDeviceDependentResourcesInternal()
{
    ...
    CHK(m_deviceResources->GetD2DDeviceContext()->CreateImageSourceFromWic(
            m_formatConvert.get(),
            m_imageSource.put()));
    ...
    {
        com_ptr<ID2D1ColorContext1> colorContext1;
        CHK(context->CreateColorContextFromDxgiColorSpace(DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020, colorContext1.put()));
        colorContext1.as(m_colorContext);
    }
    // If the image contains an embedded color profile, use it.
    else if (m_imageInfo.numProfiles >= 1)
    {
        CHK(context->CreateColorContextFromWicColorContext(
    ...
    {
        CHK(context->CreateColorContext(
...
ID2D1TransformedImageSource* ImageLoader::GetLoadedImage(float zoom)
{
    ...
    CHK(m_deviceResources->GetD2DDeviceContext()->CreateTransformedImageSource(
            m_imageSource.get(),
            &props,
            source.put()));
...
bool ImageLoader::IsImageXboxHdrScreenshot(IWICBitmapSource* source)
{
    ...
    if (FAILED(frame->GetMetadataQueryReader(metadata.put())))
    {
