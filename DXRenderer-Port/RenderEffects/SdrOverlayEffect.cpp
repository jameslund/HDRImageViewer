    ...
    )
{
    ...
    std::unique_ptr<DXR::BasicReaderWriter> reader = std::make_unique<DXR::BasicReaderWriter>();
    std::vector<unsigned char> data;

    try
    {
        // CSO files are stored in the project subfolder in the app install location.
        data = reader->ReadData(L"DXRenderer\\SdrOverlayEffect.cso");
    }
    ...
    HRESULT hr = pEffectContext->LoadPixelShader(GUID_SdrOverlayPixelShader, data.data(), static_cast<UINT>(data.size()));
